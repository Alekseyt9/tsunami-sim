...
def main() -> None:
    ...
    start_frame = int(metrics[0]["frame"])
    if [row["frame"] for row in metrics] != list(range(start_frame, start_frame + expected)):
        raise AssertionError("frame metrics are missing or out of order")

    basename = "deluge_v3"
    config_path = output / "config_used.json"
    if config_path.exists():
        basename = str(json.loads(config_path.read_text(encoding="utf-8")).get("output_basename", basename))
    segment = f"_segment_{start_frame:05d}" if start_frame else ""
    video = output / f"{basename}{segment}.mp4"
    ...
    print(
        f"PASS: frames {start_frame}-{start_frame + expected - 1} "
        f"({expected} frames / {float(probe['format']['duration']):.4f} s); "
